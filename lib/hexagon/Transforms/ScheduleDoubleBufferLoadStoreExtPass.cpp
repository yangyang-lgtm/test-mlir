...
bool isLoadExt(Operation *op) { return isa<memref_ext::LoadExOp>(op); } // 这个判断隔离出输入预取操作，便于调试或扩展调度逻辑。
bool isStoreExt(Operation *op) { return isa<memref_ext::StoreExOp>(op); } // 这个判断隔离出输出写回操作，便于调试或扩展调度逻辑。
...
bool hasConflictWhenCrossing(Operation *op, Value source, Value target, // 移动跨过中间操作前，必须证明中间操作不读写相关源/目标内存。
  ...
  for (const MemoryEffects::EffectInstance &effect : effects) {
    ...
    if (writes && // 写操作会结束此前读者的开放区间，并成为新的最近写。
      ...
      return true;
    if ((reads || writes) && // 写操作会结束此前读者的开放区间，并成为新的最近写。
...
public:
  ...
  SmallVector<memref_ext::LoadExOp> getLoads() const {
    SmallVector<memref_ext::LoadExOp> loads;
    for (const ExtNode &node : nodes)
      if (node.role == ExtScheduleRole::Load) // 调度阶段只对 load_ex 做前移优化，compute/store 节点不能混入这个候选列表。
        loads.push_back(cast<memref_ext::LoadExOp>(node.op));
    return loads;
  }

  SmallVector<memref_ext::StoreExOp> getStores() const {
    ...
    for (const ExtNode &node : nodes)
      if (node.role == ExtScheduleRole::Store) // 调度阶段只对 store_ex 做后移优化，load/compute 节点不能混入这个候选列表。
        stores.push_back(cast<memref_ext::StoreExOp>(node.op));
    return stores;
  }
  ...
  std::optional<ExtMove> getLoadMove(memref_ext::LoadExOp load) const {
    ...
    llvm::SetVector<Operation *> movableOps;
    if (!collectMovableDefs(load.getPtr(), &block, anchor, load, // 插入点没有带来真实位置变化时跳过，避免无意义改写。
                            movableOps) ||
        !collectMovableDefs(load.getValidSize(), &block, anchor, load, // 递归收集 load 使用的本地定义，保证前移后操作数仍然支配 load。
                            movableOps) ||
        !collectMovableDefs(load.getOther(), &block, anchor, load, // 递归收集 load 使用的本地定义，保证前移后操作数仍然支配 load。
                            movableOps) ||
        !collectMovableDefs(load.getTarget(), &block, anchor, load, // 递归收集 load 使用的本地定义，保证前移后操作数仍然支配 load。
                            movableOps))
      return std::nullopt;
    ...
    for (Operation *current = anchor; current && current != load;
      ...
      if (movableOps.contains(current)) // 这些地址/视图定义会和 load 一起搬走，检查它们等于把同一组移动内部误判成冲突。
        continue;
      if (hasConflictWhenCrossing(current, load.getPtr(), load.getTarget(), // 前移 load 会跨过 current；若 current 读写源/目标别名内存，提前 DMA 会改变可见顺序。
                                  aliasAnalysis))
        return std::nullopt;
    }
  ...
  std::optional<ExtMove> getStoreMove(memref_ext::StoreExOp store) const {
    ...
    for (Operation *current = store->getNextNode(); current &&
      ...
      if (hasConflictWhenCrossing(current, store.getValue(), store.getPtr(), // 后移 store 会跨过 current；若 current 读写待写回 tile 或目的地址，写回顺序不能交换。
                                  aliasAnalysis))
        return std::nullopt;
    }
...
bool scheduleBlock(Block &block, AliasAnalysis &aliasAnalysis) { // 在一个 loop body 内反复调整 load/store，直到局部顺序稳定。
  ...
  while (localChanged) { // 反复重建依赖图并移动，直到本轮不再产生变化。
    ...
    for (memref_ext::LoadExOp load : analysis.getLoads()) { // 按当前 IR 顺序尝试前移每个 load_ex。
    ...
    for (memref_ext::StoreExOp store : llvm::reverse(analysis.getStores())) { // 从后往前尝试后移 store_ex，减少互相干扰。
