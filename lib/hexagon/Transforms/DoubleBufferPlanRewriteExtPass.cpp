...
bool customIsMemoryEffectFree(Operation *op) {
  if (isa<triton::BlockDescAdvanceOp>(op))
    return true;
...
struct Plan { // Plan 汇总分析结果，rewritePlan 只消费这个结构做模板化重写。
  ...
  SmallVector<Operation *> prefetchAddressChain; // 按依赖顺序列出计算下一轮地址的纯 op（如 block_desc_advance），由 yield 操作数反向追溯得到。
  DenseSet<Operation *> prefetchAddressChainSet; // 与 prefetchAddressChain 相同的 op 集合，用于 steadyOps 循环中 O(1) 跳过。
...
struct ExtTileDataFlowAnalysis {
  ...
  bool run(Plan &plan) { // tile 收集、依赖建图和分类都成功才允许进入结构化重写。
    plan.loop = loop;
    ...
    for (Operation &op : loop.getBody()->without_terminator()) { // 只分析 loop body 的实际操作，terminator 不参与 tile 数据流。
      if (std::optional<TransferLike> transfer = getTransferLike(&op)) {
        ...
        if (!root || !isAvailableBefore(root, loop, transfer->op)) // transfer 访问的 tile 必须支配该操作，否则克隆后可能引用未定义值。
          return false;
        if (!addTile(root, transfer->isLoad() ? transfer->op : nullptr, plan)) // load 记录预取模板，store-only tile 只记录 root 供 currentMapping 替换。
          return false;
      }
    }
  ...
  bool buildNodes(Plan &plan) { // 把 loop body 的 tile 访问转成依赖图，保证重写后的顺序等价。
    ...
    for (Operation &op : loop.getBody()->without_terminator()) { // 只分析 loop body 的实际操作，terminator 不参与 tile 数据流。
      ...
      } else if (!customIsMemoryEffectFree(&op) &&
  ...
  bool classify(Plan &plan) { // 收集 steadyOps（按程序顺序）并构建 prefetchAddressChain（从 yield 操作数反向追溯地址计算 op）。
    Block *body = loop.getBody();
    ...
    for (Operation &op : body->without_terminator()) {
      if (isExcluded(&op))
        continue;
      if (!customIsMemoryEffectFree(&op) || getTransferLike(&op))
        markNeeded(&op);
    }
    ...
    // 构建 prefetch 地址链：从 yield 操作数沿纯 op 反向追溯，找出计算下一轮地址的 op 集合。
    // 这些 op（如 block_desc_advance）必须在 prefetch 之前克隆，让 prefetch 能用 advance 后的值发起 DMA。
    DenseSet<Operation *> chainSet;
    SmallVector<Operation *> traceWorklist;
    auto isChainCandidate = [&](Operation *op) -> bool {
      if (!op || op->getBlock() != body || op == terminator)
        return false;
      if (!needed.contains(op)) // 只考虑 steadyOps 中已有的 op，外部定义无需克隆。
        return false;
      if (getTransferLike(op)) // load/store 由专门路径处理，不属于地址计算。
        return false;
      if (!customIsMemoryEffectFree(op)) // 有内存副作用的 op 不能提前，否则会改变 tile 数据流时序。
        return false;
      return true;
    };
    for (Value operand : terminator->getOperands()) {
      Operation *def = operand.getDefiningOp();
      if (def && isChainCandidate(def) && chainSet.insert(def).second)
        traceWorklist.push_back(def);
    }
    while (!traceWorklist.empty()) {
      Operation *op = traceWorklist.pop_back_val();
      for (Value operand : op->getOperands()) {
        Operation *def = operand.getDefiningOp();
        if (def && isChainCandidate(def) && chainSet.insert(def).second)
          traceWorklist.push_back(def);
    ...
    for (Operation *op : plan.steadyOps) // 按 steadyOps 程序顺序过滤，保证依赖顺序。
      if (chainSet.contains(op))
        plan.prefetchAddressChain.push_back(op);
    plan.prefetchAddressChainSet = std::move(chainSet);
  ...
  void print(const Plan &plan, raw_ostream &os) { // 调试输出让重写前的数据流形态可见，方便排查误分类。
    ...
    os << "prefetch address chain:\n";
    for (auto [i, op] : llvm::enumerate(plan.prefetchAddressChain)) {
      os << "  [" << i << "] ";
...
bool rewritePlan(IRRewriter &rewriter, Plan &plan, int64_t planId) { // 用模板替换原始 loop：prologue 预取首轮 → ping/pong loop（地址链 → prefetch → compute/store）。
  ...
  Value trueValue = arith::ConstantOp::create(rewriter, loc, rewriter.getBoolAttr(true)); // 布尔常量，用于 XOR 翻转 ping/pong 状态。
  ...
  Value hasFirst = // 空循环不能执行 prologue 预取，避免越界 DMA。
      arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::slt, first,
                            loop.getUpperBound());
  auto prologue = scf::IfOp::create(rewriter, loc, hasFirst, /*withElseRegion=*/false);
  ...
  prologue->setAttr(kPlanPrologueAttr, UnitAttr::get(context));
  {
    ...
    for (auto [index, iterArg] : llvm::enumerate(loop.getRegionIterArgs()))
      mapping.map(iterArg, loop.getInitArgs()[index]); // prologue 在 loop 外部，iter_arg 必须映射到 init_arg 以避免引用即将被删除的 block argument。
    for (TileInfo tile : plan.tiles)
      if (tile.load)
        cloneTransfer(tile.load, rewriter, mapping, sourceBlock, kPrefetchRole);
  }
  ...
  dbLoop->setAttr(kPlanIdAttr, idAttr);
  {
    ...
    Value next = arith::XOrIOp::create(rewriter, loc, cur, trueValue); // 翻转 ping/pong 状态，每轮切换一次。
    ...
    IRMapping currentMapping; // 当前轮映射：iv→新 iv，tile root→本轮 ping/pong 视图，iter_arg→新循环 iter_arg。
    ...
    // 克隆 prefetch 地址链（如 block_desc_advance），由 classify 通过 yield 操作数反向追溯确定。
    // 提前克隆让 prefetch 能用 advance 后的值发起下一轮 DMA；后续 steadyOps 循环中跳过它们避免重复。
    for (Operation *op : plan.prefetchAddressChain)
    ...
    prefetch->setAttr(kPlanPrefetchAttr, UnitAttr::get(context)); // 标记循环内下一轮预取区域，帮助 DMA lowering 做角色区分。
    {
      ...
      for (auto [index, iterArg] : llvm::enumerate(loop.getRegionIterArgs()))
        mapping.map(iterArg, currentMapping.lookupOrDefault(origYieldForPrefetch.getOperand(index))); // yield 操作数即下一轮 iter_arg；地址链已提前克隆，lookupOrDefault 取到 advance 后的值。
      for (TileInfo tile : plan.tiles)
        if (tile.load)
          cloneTransfer(tile.load, rewriter, mapping, sourceBlock, kPrefetchRole);
    }
    ...
    for (Operation *op : plan.steadyOps) { // 按原顺序克隆 compute/store/scratch，保持副作用次序与数据流语义。
      if (plan.prefetchAddressChainSet.count(op)) // 已在 prefetch 前克隆的地址链 op 不重复克隆。
        continue;
    ...
    for (Value operand : origYield.getOperands())
      yieldValues.push_back(currentMapping.lookupOrDefault(operand)); // 原循环携带值经当前轮映射后 yield 出去。
...
struct HexagonDoubleBufferPlanRewriteExtPass
  ...
  void runOnOperation() override {
    ...
    for (scf::ForOp loop : loops) {
      ...
      rewritePlan(rewriter, plan, nextPlanId++);
    }
  }
};
