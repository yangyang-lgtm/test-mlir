...
struct Plan { // Plan 汇总分析结果，rewritePlan 只消费这个结构做模板化重写。
  ...
  SmallVector<Operation *> computes; // 计算操作保持原顺序克隆，确保当前轮数据流语义不变。
  SmallVector<Operation *> stores; // store 操作延后到计算之后克隆，负责写回当前轮结果。
...
struct ExtTileDataFlowAnalysis {
  ...
  bool collectTiles(Plan &plan) { // 先找到所有参与 load/store_ext 的本地 tile，重写时才能统一替换成新 buffer。
    for (Operation &op : loop.getBody()->without_terminator()) { // 只分析 loop body 的实际操作，terminator 不参与 tile 数据流。
      if (auto load = dyn_cast<memref_ext::LoadExOp>(&op)) { // load_ex 是输入预取点，它写入本地 tile，需要作为调度边界分析。
        Value root = findMemoryRoot(load.getTarget()); // 根内存是原 loop 中所有视图映射到 ping/pong buffer 的键。
        if (!root || !isAvailableBefore(root, loop, load)) // load 的目标 tile 必须能追到并支配该 load；否则 prologue 克隆 load 时无法把目标替换成 ping buffer。
          return false;
        if (!addTile(root, load, plan)) // 一个 tile 只能对应一个预取 load；多个 load 写同一根内存会让 prologue/next-prefetch 模板无法确定来源。
          return false;
        continue;
      }
      if (auto load = dyn_cast<memref_ext::BlockDescriptorLoadOp>(&op)) {
        Value root = findMemoryRoot(load.getTarget());
        if (!root || !isAvailableBefore(root, loop, load))
          return false;
        if (!addTile(root, load, plan))
          return false;
        continue;
      }
      ...
      if (auto store = dyn_cast<memref_ext::StoreExOp>(&op)) { // store_ex 是输出写回点，它读取本地 tile，需要尽量推迟但不能越过依赖。
        Value root = findMemoryRoot(store.getValue()); // 根内存是原 loop 中所有视图映射到 ping/pong buffer 的键。
        if (!root || !isAvailableBefore(root, loop, store)) // store 读取的 tile 必须能映射到当前轮 buffer；否则克隆 store 后会读原始 tile 或未定义值。
          return false;
        if (!addTile(root, nullptr, plan)) // 即使没有对应 load，store 源也必须作为计划 tile 记录，保证后续 currentMapping 能替换它。
          return false;
      } else if (auto store = dyn_cast<memref_ext::BlockDescriptorStoreOp>(&op)) {
        Value root = findMemoryRoot(store.getValue());
        if (!root || !isAvailableBefore(root, loop, store))
          return false;
        if (!addTile(root, nullptr, plan))
          return false;
  ...
  bool buildNodes(Plan &plan) { // 把 loop body 的 tile 访问转成依赖图，保证重写后的顺序等价。
    ...
    for (Operation &op : loop.getBody()->without_terminator()) { // 只分析 loop body 的实际操作，terminator 不参与 tile 数据流。
      ...
      if (auto load = dyn_cast<memref_ext::LoadExOp>(&op)) { // load_ex 是输入预取点，它写入本地 tile，需要作为调度边界分析。
        ...
        if (auto target = getTileIndex(load.getTarget(), plan)) // load_ex 的目标是计划 tile 时，它在数据流图里代表“生产本地 tile”的写节点。
          node.accesses[*target] |= TileWrite; // 写标志用于建立 WAW 和对历史读的 WAR 依赖。
      } else if (auto load = dyn_cast<memref_ext::BlockDescriptorLoadOp>(&op)) {
        node.isLoad = true;
        if (auto target = getTileIndex(load.getTarget(), plan))
          node.accesses[*target] |= TileWrite;
      } else if (auto load = dyn_cast<memref_ext::BlockDescLoadOp>(&op)) {
        node.isLoad = true;
        if (auto target = getTileIndex(load.getTarget(), plan))
          node.accesses[*target] |= TileWrite;
      } else if (auto store = dyn_cast<memref_ext::StoreExOp>(&op)) {
        node.isStore = true; // store 节点会被克隆到当前轮计算之后执行写回。
        if (auto value = getTileIndex(store.getValue(), plan)) // store_ex 的源是计划 tile 时，它在数据流图里代表“消费本地 tile 后写回”的读节点。
          node.accesses[*value] |= TileRead; // 读标志用于建立对最近写的 RAW 依赖。
      } else if (auto store = dyn_cast<memref_ext::BlockDescriptorStoreOp>(&op)) {
        node.isStore = true;
        if (auto value = getTileIndex(store.getValue(), plan))
          node.accesses[*value] |= TileRead;
      } else if (auto store = dyn_cast<memref_ext::BlockDescStoreOp>(&op)) {
        node.isStore = true;
        if (auto value = getTileIndex(store.getValue(), plan))
          node.accesses[*value] |= TileRead;
  ...
  bool classify(Plan &plan) { // 把数据流节点拆成 rewritePlan 需要的 load/compute/store 三类。
    for (TileNode &node : plan.nodes) {
      if (node.isLoad) // load_ex 已由 prologue 和 loop 内 next-prefetch 克隆，不能再作为 compute 克隆一次。
        continue;
      if (node.isStore) { // store_ex 要在本轮 compute 后克隆为写回阶段，和普通 compute 分开收集。
        Value value;
        if (auto store = dyn_cast<memref_ext::StoreExOp>(node.op))
          value = store.getValue();
        else if (auto store = dyn_cast<memref_ext::BlockDescriptorStoreOp>(node.op))
          value = store.getValue();
        else if (auto store = dyn_cast<memref_ext::BlockDescStoreOp>(node.op))
          value = store.getValue();
        if (!value || !getTileIndex(value, plan)) // store 源若不是计划 tile，currentMapping 无法替换它，重写后的 store 会失去正确数据来源。
          return false; // 当前 IR 形态无法证明安全，放弃这次优化以保持语义。
        plan.stores.push_back(node.op);
        continue;
      }
      node.isCompute = true; // compute 节点会被克隆到 double-buffer loop 的等待之后。
      plan.computes.push_back(node.op);
    }
    return !plan.computes.empty() && (plan.isReduction || !plan.stores.empty()); // 模板至少需要 compute；非 reduce 还必须有 store，否则重写后没有明确写回阶段。
  }
...
bool rewritePlan(IRRewriter &rewriter, Plan &plan, int64_t planId) { // 用 prologue 预取首轮、loop 内预取下一轮的模板替换原始 loop。
  ...
  auto dbLoop = scf::ForOp::create(rewriter, loc, loop.getLowerBound(), // 新循环携带 ping/pong 状态，替代原始 loop 执行 steady-state。
                                   loop.getUpperBound(), loop.getStep(),
                                   ValueRange{trueValue}); // 循环初始选择 ping buffer，随后每轮翻转。
  dbLoop->setAttr(kPlanIdAttr, idAttr); // 给新生成的 prologue/loop 打同一个 id，方便后续 pass 识别同一计划。
  {
    ...
    for (auto [index, pingBuffer] : llvm::enumerate(pingBuffers)) { // 为每个 tile 生成当前轮和下一轮视图，支撑 ping/pong 切换。
      ...
      nextViews.push_back( // 下一轮视图提供给提前发起的 prefetch。
          selectBuffer(rewriter, loc, cur, pongBuffer, pingBuffer)); // 用循环携带的布尔状态在 ping 和 pong 之间选择当前轮 buffer。
    }
    ...
    for (Operation *compute : plan.computes) // 按原顺序克隆计算，依赖已经保证它们看到当前轮数据。
      cloneMappedOp(compute, rewriter, currentMapping, sourceBlock, // 按当前轮映射克隆 compute/store/load，保持原操作语义但替换 tile。
                    /*markCompute=*/true);
    for (Operation *store : plan.stores) // 计算完成后克隆写回，保证 store 读取最终 tile 内容。
      cloneTransfer(store, rewriter, currentMapping, sourceBlock, // 传输操作需要额外角色属性，供后续 lowering 区分 DMA start/store。
                    kDB2StoreRole);
    scf::YieldOp::create(rewriter, loc, next); // 把翻转后的状态传给下一轮，完成 ping/pong 轮换。
  }
