...
#include "hexagon/Dialect/MemRefExt/IR/MemRefExtDialect.h" // 生成 memref_ext.dma_handle/dma_load_ex/dma_wait。
...
Value createDoubleBufferHandles(Location loc, IRRewriter &rewriter) {
  return memref_ext::CreateAndInitHandlesOp::create(
             rewriter, loc,
             memref_ext::DoubleBufferDmaHandlesType::get(
                 rewriter.getContext(), 2))
      .getHandles();
}

Value createDoubleBufferFetchData(Location loc, IRRewriter &rewriter) {
  return memref_ext::CreateDoubleBufferFetchDataOp::create(
             rewriter, loc,
             memref_ext::DoubleBufferDmaFetchDataType::get(
                 rewriter.getContext(), 2))
      .getFetchData();
}
...
LogicalResult replaceLoadWithDMAStart(IRRewriter &rewriter,
                                      ...
                                      Value handle,
                                      Value fetchData) { // 把一个 prefetch load_ex 替换为 dma_load_ex。
  // 保留 load_ex 的地址语义，只替换传输触发方式：
  //   load_ex(ptr, valid, other, target)
  // 变成：
  //   if valid > 0 { dma_load_ex(ptr, valid, other, target, handle) }
  // 对应的 dma_wait 在 schedule 层统一插入。
  rewriter.setInsertionPoint(load); // 新的 guard 和 dma_load_ex 放在原 load_ex 位置。
  ...
  if (failed(createIfPositive(rewriter, load.getLoc(), numElements, [&] { // 只在 valid > 0 时发起 DMA。
        memref_ext::DmaStartExOp::create( // 创建异步 DMA start。
            ...
            load.getIsOtherValidAttr(), handle, fetchData); // 复用 other-valid 属性，并绑定选中的 DMA handle。
        return success(); // then body 构造成功。
      })))
    return failure(); // guard 或 dma_load_ex 构造失败时向上传递失败。
  rewriter.eraseOp(load); // 原 load_ex 已被 guarded dma_load_ex 替代。
  return success(); // 当前 load lowering 完成。
}

LogicalResult replaceBlockDescriptorLoadWithDMAStart(
    IRRewriter &rewriter, memref_ext::BlockDescriptorLoadOp load,
    Value handle, Value fetchData) {
  rewriter.setInsertionPoint(load);
  memref_ext::DmaBlockDescriptorLoadOp::create(
      rewriter, load.getLoc(), load.getDesc(), load.getBoundaryCheckAttr(),
      load.getPaddingAttr(), load.getTarget(), handle, fetchData);
  rewriter.eraseOp(load);
  return success();
}

LogicalResult replaceBlockDescLoadWithDMAStart(
    IRRewriter &rewriter, memref_ext::BlockDescLoadOp load, Value handle,
    Value fetchData) {
  rewriter.setInsertionPoint(load);
  memref_ext::DmaBlockDescriptorLoadOp::create(
      rewriter, load.getLoc(), load.getDesc(), load.getBoundaryCheckAttr(),
...
void collectRoleBlockDescriptorLoads(
    Block &block, SmallVectorImpl<memref_ext::BlockDescriptorLoadOp> &ops) {
...
void collectRoleBlockDescLoads(
    Block &block, SmallVectorImpl<memref_ext::BlockDescLoadOp> &ops) {
  for (Operation &op : block.without_terminator())
...
struct PlannedExtSchedule { // 保存一个 PlanRewriteExt 生成的完整 double-buffer 计划。
  ...
  SmallVector<memref_ext::BlockDescriptorLoadOp> prologueBlockDescriptorLoads;
  SmallVector<memref_ext::BlockDescriptorLoadOp> nextBlockDescriptorLoads;
  SmallVector<memref_ext::BlockDescLoadOp> prologueBlockDescLoads;
  SmallVector<memref_ext::BlockDescLoadOp> nextBlockDescLoads;
...
bool extractSchedule(scf::ForOp kernel, PlannedExtSchedule &schedule) { // 从一个候选 kernel 中提取 planned schedule。
  ...
  // kernel loop 的首个 iter_arg 必须是 i1 ping/pong 状态；其后可携带原 loop 的 iter_args。
  if (!idAttr || kernel.getRegionIterArgs().empty() ||
  ...
  collectRoleBlockDescriptorLoads(thenBlock(schedule.prologue),
                                  schedule.prologueBlockDescriptorLoads);
  collectRoleBlockDescriptorLoads(thenBlock(schedule.prefetch),
                                  schedule.nextBlockDescriptorLoads);
  collectRoleBlockDescLoads(thenBlock(schedule.prologue),
                            schedule.prologueBlockDescLoads);
  collectRoleBlockDescLoads(thenBlock(schedule.prefetch),
                            schedule.nextBlockDescLoads);
  collectRoleStores(*kernel.getBody(), schedule.stores); // 收集写回 store，用于确认计划完整。

  // prologue loads 和 steady-state loads 按 tile 成对。reduce loop 允许没有循环内 store。
  size_t prologueLoadCount = schedule.prologueLoads.size() +
                             schedule.prologueBlockDescriptorLoads.size() +
                             schedule.prologueBlockDescLoads.size();
  size_t nextLoadCount = schedule.nextLoads.size() +
                         schedule.nextBlockDescriptorLoads.size() +
                         schedule.nextBlockDescLoads.size();
  return prologueLoadCount != 0 && prologueLoadCount == nextLoadCount;
}
...
LogicalResult lowerSchedule(IRRewriter &rewriter,
  ...
  size_t loadCount = schedule.prologueLoads.size() +
                     schedule.prologueBlockDescriptorLoads.size() +
                     schedule.prologueBlockDescLoads.size();
  SmallVector<Value> fetchData;
  fetchData.reserve(loadCount);
  for (size_t index = 0; index < loadCount; ++index) {
    fetchData.push_back(createDoubleBufferFetchData(loc, rewriter));
  }
  ...
  SmallVector<Value> pingFetchData;
  pingFetchData.reserve(loadCount);
  for (Value pair : fetchData)
    pingFetchData.push_back(selectDMAFetchData(rewriter, loc, pair, trueValue));
  size_t fetchIndex = 0;
  for (memref_ext::LoadExOp load : schedule.prologueLoads) // 逐个 lowering 首轮预取 load。
    if (failed(replaceLoadWithDMAStart(rewriter, load, pingLoadHandle,
                                       pingFetchData[fetchIndex++]))) // prologue load 绑定 ping handle。
      return failure(); // 任一 load lowering 失败则整个 schedule lowering 失败。
  for (memref_ext::BlockDescriptorLoadOp load :
       schedule.prologueBlockDescriptorLoads)
    if (failed(replaceBlockDescriptorLoadWithDMAStart(rewriter, load,
                                                      pingLoadHandle,
                                                      pingFetchData[fetchIndex++])))
      return failure();
  for (memref_ext::BlockDescLoadOp load : schedule.prologueBlockDescLoads)
    if (failed(
            replaceBlockDescLoadWithDMAStart(rewriter, load, pingLoadHandle,
                                             pingFetchData[fetchIndex++])))
      return failure();
  ...
  nextFetchData.reserve(loadCount);
  for (Value pair : fetchData)
    nextFetchData.push_back(selectDMAFetchData(rewriter, loc, pair, next));
  ...
  for (memref_ext::LoadExOp load : schedule.nextLoads) // lowering loop 内下一轮预取 load。
    if (failed(replaceLoadWithDMAStart(rewriter, load, nextLoadHandle,
                                       nextFetchData[fetchIndex++]))) // next prefetch 绑定 next handle。
      return failure(); // 任一 load lowering 失败则整个 schedule lowering 失败。
  for (memref_ext::BlockDescriptorLoadOp load :
       schedule.nextBlockDescriptorLoads)
    if (failed(replaceBlockDescriptorLoadWithDMAStart(rewriter, load,
                                                      nextLoadHandle,
                                                      nextFetchData[fetchIndex++])))
      return failure();
  for (memref_ext::BlockDescLoadOp load : schedule.nextBlockDescLoads)
    if (failed(
            replaceBlockDescLoadWithDMAStart(rewriter, load, nextLoadHandle,
                                             nextFetchData[fetchIndex++])))
      return failure();
  ...
  void runOnOperation() override { // pass 主入口。
    ...
    for (scf::ForOp kernel : kernels) { // 按收集顺序处理每个候选 kernel。
      ...
      if (!extractSchedule(kernel, schedule)) // 校验并提取 prologue/prefetch/load/store。
        continue; // 不完整计划保持原样。
      if (failed(lowerSchedule(rewriter, schedule))) { // 执行 load_ex 到 dma_load_ex 的 lowering。
